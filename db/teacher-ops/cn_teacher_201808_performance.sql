...
drop table if exists tmp_assing_teacher_t1;
create temporary table tmp_assing_teacher_t1
(
    job_number varchar(20) CHARACTER SET utf8 COLLATE utf8_unicode_ci
    ,order_id int
);
insert into tmp_assing_teacher_t1
values
('0688190',1)
,('0688228',2)
,('0688247',3)
...
drop table if exists tmp_assing_teacher_t2;
create temporary table tmp_assing_teacher_t2
(
    job_number varchar(20) CHARACTER SET utf8 COLLATE utf8_unicode_ci
    ,order_id int
    ,assign_teacher int
...
insert into tmp_assing_teacher_t2
select
	t1.job_number
	,t1.order_id
	,t2.masterid
	,t2.truename
from tmp_assing_teacher_t1 t1
left join
	newuuabc.admin 	t2
...
-- 7月30日带1v1学员数：
   ...
   -- # 拉取数据当天1:1课时有剩余或者指定日期有上过课的学生，按这些学生取拉取数据时的督导老师
    ...
    left join
        ...
        and date(convert_tz(from_unixtime(start_time), '+00:00', '+08:00')) between 20180730 and 20180730  -- 上课日期
        )   t2
...
--    8月26日这个学生是指派给这个督导的
    ...
    left join
        ...
        -- and disabled = 0  -- 正常
        and class_appoint_course_id = 0  -- 约课id
        and date(convert_tz(from_unixtime(start_time), '+00:00', '+08:00')) between 20180826 and 20180826  -- 上课日期
        )   t2
    on  t1.student_id = t2.student_id
    group by t0.assign_teacher
        ,t0.truename
...
--    7月30日这个学生是指派给这个督导的
    ...
    left join
        ...
        and date(convert_tz(from_unixtime(start_time), '+00:00', '+08:00')) between 20180730 and 20180730
        union
        select distinct
            student_id
        from classbooking.student_class
        -- where status = 3  -- 已完成
        -- and date(from_unixtime(start_time)) between 20180730 and 20180730
        where date(from_unixtime(start_time)) between 20180730 and 20180730
        )   t2
    on  t1.student_id = t2.student_id
    group by t0.assign_teacher
        ,t0.truename
...
--    8月26日这个学生是指派给这个督导的
    ...
    left join
        ...
        -- and disabled = 0  -- 正常
        and class_appoint_course_id <> 0  -- 1:4
        and date(convert_tz(from_unixtime(start_time), '+00:00', '+08:00')) between 20180826 and 20180826
        union
        select distinct
            student_id
        from classbooking.student_class
        -- where status = 3  -- 已完成
        -- and date(from_unixtime(start_time)) between 20180826 and 20180826
        where date(from_unixtime(start_time)) between 20180826 and 20180826
        )   t2
    on  t1.student_id = t2.student_id
    group by t0.assign_teacher
        ,t0.truename
...
--    每天指派给这个督导的学生完成的课程数之和（有上课就算完成）
    ...
    and date(convert_tz(from_unixtime(t2.start_time), '+00:00', '+08:00')) between 20180730 and 20180826
    group by t0.assign_teacher
    order by t0.order_id
    ;
	...
	left join
		...
		and date(convert_tz(from_unixtime(start_time), '+00:00', '+08:00')) between 20180730 and 20180826
	...
	group by t1.assign_teacher, t3.truename
...
--    7月30号至8月26号，有合同付费金额大于0的学员，合计这些学员的1：4完成课时
    ...
    left join
        ...
        from classbooking.student_class
        where status = 3
        and date(from_unixtime(start_time)) between 20180730 and 20180826
        )     t2
...
--
    ...
    left join
        ...
        inner join
            newuuabc.student_user t5
        on  t1.student_id = t5.id
        and t5.flag = 1
        and t5.disable = 0
        where t1.status = 3
        and date(from_unixtime(t1.start_time)) between 20180730 and 20180826
        ...
        on  t3.template_id = t4.id
        and t4.id in (295, 404, 349)
        where date(convert_tz(from_unixtime(t1.start_time), '+00:00', '+08:00')) between 20180730 and 20180826
...
-- 所带学员完成素质课课时数
    ...
    and t2.status = 3
    and t2.class_type = 5
    and date(from_unixtime(t2.start_time)) between 20180730 and 20180826
    group by t0.assign_teacher
    ;
...
--    7月30号至8月26号，有进行续费的学员，金额总和
    ...
    and t2.attribute = 2
    and ((t2.status = 3
        and date(convert_tz(from_unixtime(update_at), '+00:00', '+08:00')) between 20180730 and 20180826
        )
        or  (t2.status = 4
            and date(convert_tz(from_unixtime(sucess_at), '+00:00', '+08:00')) between 20180730 and 20180826
            )
        )
    left join
        (select

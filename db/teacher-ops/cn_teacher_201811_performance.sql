...
drop table if exists bi.tmp_teacher_20181221;
create table bi.tmp_teacher_20181221
(
...
insert into bi.tmp_teacher_20181221
select
    t2.job_number
    ,t1.order_id
...
drop table if exists bi.tmp_student_20181221;
create table bi.tmp_student_20181221
(
    student_id int
...
insert into bi.tmp_student_20181221
select
    id as student_id
...
where partition_key between 20181026 and 20181125
;
...
-- 20181026带1v1学员数：
   -- 20181026这个学生是指派给这个督导的

   -- # 拉取数据当天1:1课时有剩余或者指定日期有上过课的学生，按这些学生取拉取数据时的督导老师


    drop table if exists teacher_1v1_student_cnt_20181026;
    create temporary table teacher_1v1_student_cnt_20181026
    (
        teacher_id int
    ...
    insert into teacher_1v1_student_cnt_20181026
    select
        t0.teacher_id
        ,t0.teacher_name
        ,t0.job_number
    ...
    left join
        ...
        and date(convert_tz(from_unixtime(start_time), '+00:00', '+08:00')) = 20181026  -- 上课日期
        )   t2
...
-- 20181125带1v1学员数：
--    20181125这个学生是指派给这个督导的
    drop table if exists teacher_1v1_student_cnt_20181125;
    create temporary table teacher_1v1_student_cnt_20181125
    (
        teacher_id int
        ,teacher_name varchar(30)
        ,job_number varchar(30)
    ...
    insert into teacher_1v1_student_cnt_20181125
    select
        t0.teacher_id
        ,t0.teacher_name
        ,t0.job_number
    ...
    left join
        ...
        -- and disabled = 0  -- 正常
        and class_appoint_course_id = 0  -- 约课id
        and date(convert_tz(from_unixtime(start_time), '+00:00', '+08:00')) = 20181125  -- 上课日期
        )   t2
    on  t1.student_id = t2.student_id
    group by t0.teacher_id
        ,t0.teacher_name
...
--    20181026这个学生是指派给这个督导的
    ...
    drop table if exists teacher_1v4_student_cnt_20181026;
    create temporary table teacher_1v4_student_cnt_20181026
    (
        teacher_id int
        ,teacher_name varchar(30)
        ,job_number varchar(30)
    ...
    insert into teacher_1v4_student_cnt_20181026
    select
        t0.teacher_id
        ,t0.teacher_name
        ,t0.job_number
    ...
    left join
        ...
        and date(convert_tz(from_unixtime(start_time), '+00:00', '+08:00')) = 20181026
        union
        select distinct
            student_id
        ...
        where date(from_unixtime(start_time)) = 20181026
        )   t2
    on  t1.student_id = t2.student_id
    group by t0.teacher_id
        ,t0.teacher_name
...
--    20181125这个学生是指派给这个督导的
    ...
    drop table if exists teacher_1v4_student_cnt_20181125;
    create temporary table teacher_1v4_student_cnt_20181125
    (
        teacher_id int
        ,teacher_name varchar(30)
        ,job_number varchar(30)
    ...
    insert into teacher_1v4_student_cnt_20181125
    select
        t0.teacher_id
        ,t0.teacher_name
        ,t0.job_number
    ...
    left join
        ...
        -- and disabled = 0  -- 正常
        and class_appoint_course_id <> 0  -- 1:4
        and date(convert_tz(from_unixtime(start_time), '+00:00', '+08:00')) = 20181125
        union
        select distinct
            student_id
        from classbooking.student_class
        ...
        where date(from_unixtime(start_time)) = 20181125
        )   t2
    on  t1.student_id = t2.student_id
    group by t0.teacher_id
        ,t0.teacher_name
...
--    每天指派给这个督导的学生完成的课程数之和（有上课就算完成）
    ...
    select
        t0.teacher_id
        ,sum(case when t2.student_id is not null then 1 else 0 end) as cnt1
    from bi.tmp_teacher_20181221 t0
    left join
        bi.tmp_student_20181221  t1
    on  t0.teacher_id = t1.teacher_id
    and t1.flag = 1
    and t1.disable = 0
    left join
        ...
        and date(convert_tz(from_unixtime(start_time), '+00:00', '+08:00')) between 20181026 and 20181125
        )     t2
    on  t1.student_id = t2.student_id
...
--   20181026 至 20181125，有合同付费金额大于0的学员，合计这些学员的1：4完成课时
    ...
    select
        t1.teacher_id
        ,sum(case when t2.student_id is not null then 1 else 0 end) as cnt1
    from bi.tmp_teacher_20181221  t1
    left join
        bi.tmp_student_20181221   t3
    ...
    left join
        ...
        from classbooking.student_class
        where status = 3
        and date(from_unixtime(start_time)) between 20181026 and 20181125
        )     t2
...
-- 优氏英语 暑期20课时免费课程包  contract_template.id = 404
    ...
    left join
        ...
        where t1.status = 3
        and date(from_unixtime(t1.start_time)) between 20181026 and 20181125
    ...
    left join
        ...
        where date(convert_tz(from_unixtime(t1.start_time), '+00:00', '+08:00')) between 20181026 and 20181125
...
--  20181026 至 20181125，有进行续费的学员，金额总和
    ...
    inner join
        ...
        and t1.attribute = 2
        and ((t1.status = 3
            and date(convert_tz(from_unixtime(t1.update_at), '+00:00', '+08:00')) between 20181026 and 20181125
            )
            or  (t1.status = 4
                and date(convert_tz(from_unixtime(t1.sucess_at), '+00:00', '+08:00')) between 20181026 and 20181125
                )
            )
        )   t2
...
--    20181026 至 20181125 付费的来自至慧渠道的学员，归属到当前督导
...
--    首次付费日期还是注册日期在统计周期内？
    ...
    inner join
        ...
        and contract_type = 1
        and ((status = 3
            and date(convert_tz(from_unixtime(update_at), '+00:00', '+08:00')) between 20181026 and 20181125
            )
            or  (status = 4
                and date(convert_tz(from_unixtime(sucess_at), '+00:00', '+08:00')) between 20181026 and 20181125
                )
            )
        group by student_id
        )   t5
...
select
    ...
    ,t1.cnt1         as '20181026带1v1学员数'
    ,t1_1.cnt1       as '20181125带1v1学员数'
    ,t2.cnt1         as '20181026带1v4学员数'
    ,t2_1.cnt1       as '20181125带1v4学员数'
...
from bi.tmp_teacher_20181221 t0
left join
    teacher_1v1_student_cnt_20181026  t1
on  t0.teacher_id = t1.teacher_id
left join
    teacher_1v1_student_cnt_20181125    t1_1
on  t0.teacher_id = t1_1.teacher_id
left join
    teacher_1v4_student_cnt_20181026  t2
on  t0.teacher_id = t2.teacher_id
left join
    teacher_1v4_student_cnt_20181125    t2_1
